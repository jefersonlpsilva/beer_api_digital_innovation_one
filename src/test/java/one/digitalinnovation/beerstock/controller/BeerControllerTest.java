...
public class BeerControllerTest {
    ...
    @Test
    void whenPUTIsCalledThenABeerIsShoudBeSave() throws Exception {
        // given
        BeerDTO expectedToSaveBeer = BeerDTOBuilder.builder().build().toBeerDTO();
        BeerDTO savedbeer = BeerDTOBuilder.builder().build().toBeerDTO();

        //when
        when(beerService.updateById(VALID_BEER_ID, expectedToSaveBeer)).thenReturn(savedbeer);
        
        // then
        mockMvc.perform(MockMvcRequestBuilders.put(BEER_API_URL_PATH + "/" + expectedToSaveBeer.getId())
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(expectedToSaveBeer)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name", is(savedbeer.getName())))
                .andExpect(jsonPath("$.brand", is(savedbeer.getBrand())))
                .andExpect(jsonPath("$.type", is(savedbeer.getType().toString())));
    }
    
    @Test
    void whenPUTIsCalledWithInvalidIdThenNotFoundStatusIsReturned() throws Exception {
        BeerDTO expectedToSaveBeer = BeerDTOBuilder.builder().build().toBeerDTO();
        expectedToSaveBeer.setId(null);
        //when
        doThrow(BeerNotFoundException.class).when(beerService).updateById(INVALID_BEER_ID, expectedToSaveBeer);
        
        // then
        mockMvc.perform(MockMvcRequestBuilders.put(BEER_API_URL_PATH + "/" + INVALID_BEER_ID)
                .contentType(MediaType.APPLICATION_JSON)
                .content(asJsonString(expectedToSaveBeer)))
                .andExpect(status().isNotFound());
    }
    ...
    void whenPATCHIsCalledToIncrementGreatherThanMaxThenBadRequestStatusIsReturned() throws Exception {
        ...
        when(beerService.increment(VALID_BEER_ID, quantityDTO.getQuantity())).thenThrow(BeerStockExceededTheMaxException.class);
    ...
    void whenPATCHIsCalledToDecrementLowerThanZeroThenBadRequestStatusIsReturned() throws Exception {
         ...
         when(beerService.decrement(VALID_BEER_ID, quantityDTO.getQuantity())).thenThrow(BeerStockExceededTheMinException.class);
